...
<template>
  <div
    ...
    <div class="space-y-2">
      <div v-for="device in devices" :key="device.serial"
        ...
        <div class="flex items-center gap-1 gap-x-4 mt-2 flex-wrap">
          <span class="text-[10px] text-white/35 flex items-center gap-1">
            <span class="w-1.5 h-1.5 rounded-full" :class="device.screenOn ? 'bg-emerald-400' : 'bg-white/20'"></span>
          ...
          <span v-if="device.type === 'wireless'" class="text-[10px] text-white/35 flex items-center gap-1">
            <Icon name="globe" class="w-3 h-3" />
            {{ device.ipAddress || '-.-.-.-' }}
          </span>

        </div>
        <div class="flex items-center gap-2 mt-2.5 ml-0">
          <AppButton variant="primary" :disabled="isScrcpyRunning" @click="emit('startScrcpy', device.serial)">
            屏幕镜像
          </AppButton>
    ...
    <div v-if="isScrcpyRunning"
      ...
      <div class="relative flex-shrink-0">
        ...
        <span class="absolute inset-0 w-2 h-2 bg-emerald-400 rounded-full animate-ping opacity-50"></span>
      </div>
    ...
    <AppButton variant="link" class="mt-4 w-full justify-center py-2 rounded-xl" @click="emit('pairNew')">
      配对新设备
    </AppButton>
  </div>
</template>
